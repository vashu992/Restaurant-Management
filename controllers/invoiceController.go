...
func GetInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := invoiceCollection.FindOne(ctx, bson.M{"invoice_id": invoice}).Decode(&invoiceId)
		defer cancel()
		if err != nil {
...
func UpdateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprintf("invoice item update failed: %s", err.Error())
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}

		c.JSON(http.StatusOK, gin.H{"message": "invoice item updated successfully"})
