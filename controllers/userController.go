...
import (
	...
	"github.com/vashu992/Restaurant-Management/helpers"
...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while ckecking for the phone number "})
			return
		}
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "user not found, login seem to be incorrect"})
			return
		}
